...
void LightShader::setShaderParameters(ID3D11DeviceContext* deviceContext, const XMMATRIX& worldMatrix, const XMMATRIX& viewMatrix, const XMMATRIX& projectionMatrix, ID3D11ShaderResourceView* texture, Light* light)
{
	...
	float outerAngleDegrees = 25.0f;
	float innerCos = cosf(XMConvertToRadians(innerAngleDegrees));
	float outerCos = cosf(XMConvertToRadians(outerAngleDegrees));
	...
	lightPtr->ambient = XMFLOAT4(0.1f, 0.1f, 0.2f, 1.0f);
	lightPtr->position = light->getPosition();
	lightPtr->innerConeAngle = innerCos;
	lightPtr->direction = light->getDirection();
	lightPtr->outerConeAngle = outerCos;
	lightPtr->attenuation = XMFLOAT3(1.0f, 0.09f, 0.032f);
